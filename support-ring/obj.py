...
class BaseObjectController(Controller):
    ...
    def GETorHEAD(self, req):
        ...
        obj_ring_support = self.app.get_object_ring_support(policy_index)
        if resp.status_int // 100 != 2 and obj_ring_support:
            partition = obj_ring_support.get_part(
                self.account_name, self.container_name, self.object_name)
            node_iter = self.app.iter_nodes(obj_ring_support, partition, self.logger,
                                        policy=policy)
            resp = self._get_or_head_response(req, node_iter, partition, policy)
